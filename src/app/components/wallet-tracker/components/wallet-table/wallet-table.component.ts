...
export class WalletTableComponent implements OnInit, OnDestroy {
    ...
    public set updateTransaction(value: WalletTransaction) {
        if (value !== undefined && value.id === this.actionManager.selected_id) {
            ...
            for (let i = this.transactionList.length - 1; i > 0; i--) {
                if (this.transactionList[i].id == this.actionManager.selected_id) {
                    update_index = i;
                    break;
                }
            }
            ...
            if (this.actionManager.selected_id === first_id) {
                this.actionManager.selected_id = this.nextId;
            }
            this.transactionStorageSrv.updateTransactions(transactionsToUpdate).then(() => {
                // reupdate the first transaction id
                transactionsToUpdate[0].rename(first_id);
                if (this.actionManager.selected_id === this.nextId) {
    ...
    private getSelectedIndex(): number {
        if (this.actionManager.selected_id <= 0) {
            return -1;
        }
    ...
    public onDeleteClicked(t: WalletTransaction) {
        if (!t || t.id !== this.actionManager.selected_id) {
            return;
        }
        ...
        let selected_id = this.actionManager.selected_id;
    ...
    public isSelected(t: WalletTransaction): boolean {
        return this.actionManager.selected_id === t.id;
    }
